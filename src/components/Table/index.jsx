...
const OptionsExport = ({ rows, columns, type }) => {
  ...
  const exportPDF = () => {
    const customWidths = type === 'post' ? [35, 150, 100, 35, 35, 35, 110] : [200, 200, 75];
    ...
    pdfMake.createPdf(docDefinition).download(type === 'post' ? "post.pdf" : "account.pdf");
  };
  ...
  const exportExcel = () => {
    ...
    link.href = url;
    link.download = type === 'post' ? 'post.xlsx' : 'account.xlsx';
  ...
  const exportCSV = () => {
    ...
    link.setAttribute("download", type === 'post' ? "post.csv" : "account.csv");
    document.body.appendChild(link);
    link.click();
  };
